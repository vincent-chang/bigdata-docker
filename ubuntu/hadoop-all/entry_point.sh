...
function exec_script()
{
  ...
  nohup /root/${script_name}.sh >/root/${script_name}.log 2>&1 &
}
...
function wait_for_service_port()
{
  ...
  until [ $result -eq 0 ]; do
    ...
    ps -ef|grep -v grep|grep ${script_name} > /dev/null
    starting=$?
...
wait_for_service_port start_node_manager "localhost:8040"
echo "[i] Starting Yarn History Server..."
wait_for_service_port start_history_server "localhost:10200"
echo "[i] Starting Hive Metastore..."
wait_for_service_port start_hive_metastore "localhost:9083"
echo "[i] Starting Hive Server..."
exec_script start_hive_server
